...
public class MultithreadedSort
{
	...
	public static void runParallelMerge(List<Integer> sortArray, int numCores, int sortType)
	{
		...
		MultithreadedMergeSortConcrete sort = new MultithreadedMergeSortConcrete(latch);
	...
	public static void runParallelMergeWOSentinel(List<Integer> sortArray, int numCores)
	{
		ExecutorService executor = Executors.newFixedThreadPool(numCores);
		CountDownLatch latch = new CountDownLatch(numCores);
		MultithreadedMergeSortWOSentinel sort = new MultithreadedMergeSortWOSentinel(latch);
		sort.setUp(sortArray, numCores);
		
		// run merge sort divided in n parts = numCores
		runParallelMergeTasksWOSentinel(executor, sort);
	...
	private static void runParallelMergeTasks (ExecutorService executor, MultithreadedMergeSortConcrete sort)
	{
		int numRemainingParts = sort.getNumRemainingParts() == 0? sort.getNumCores(): sort.getNumRemainingParts()/2;
		
		CountDownLatch latch = new CountDownLatch(numRemainingParts);
		sort.resetLatch(latch);
		...
		try
		{
			latch.await(); // await for merge to finish
	...
	private static void runParallelMergeTasksWOSentinel (ExecutorService executor, MultithreadedMergeSortWOSentinel sort)
	{
		int numRemainingParts = sort.getNumRemainingParts() == 0? sort.getNumCores(): sort.getNumRemainingParts()/2;
...
//abstract class MultithreadedMerge
class MultithreadedMergeSortConcrete implements Runnable
{
	...
	public MultithreadedMergeSortConcrete(CountDownLatch latch)
	{
		mLatch = latch;
	}
	...
	public MultithreadedMergeSortConcrete setUp(List<Integer> sortArray, int numCores)
	{
...
class MultithreadedMergeSortWOSentinel implements Runnable
{
	...
	public MultithreadedMergeSortWOSentinel(CountDownLatch latch)
	{
		mLatch = latch;
	}
	...
	public MultithreadedMergeSortWOSentinel setUp(List<Integer> sortArray, int numCores)
	{
		mSortArray = sortArray;
		mNumCores = numCores;
		mRunCount = 0;
...
class MultithreadedMergeSortOptimized implements Runnable
{
	...
	public MultithreadedMergeSortOptimized(CountDownLatch latch)
	{
		mLatch = latch;
	}
	...
	public MultithreadedMergeSortOptimized setUp(List<Integer> sortArray, int numCores)
	{
		mSortArray = sortArray;
		mNumCores = numCores;
		mRunCount = 0;
